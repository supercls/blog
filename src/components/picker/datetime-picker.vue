<template>
  <mt-popup v-model="visible" :closeOnClickModal="closeOnClickModal" position="bottom" class="mint-datetime">
    <mt-picker
      ...
      ref="picker"
      show-toolbar>
      <span class="mint-datetime-action mint-datetime-cancel" style="text-align: left;padding:  0 5%;width: 40%" @click="visible = false;$emit('cancel')">{{ cancelText }}</span>
      <span class="mint-datetime-action mint-datetime-confirm" style="text-align: right;padding:  0 5%;width: 40%" @click="confirm">{{ confirmText }}</span>
    </mt-picker>
  </mt-popup>
</template>
